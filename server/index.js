...
function toFilterField(field, sqlType, isNullable) {
  return {
    name: field,
    sqlType,
    required: false,
...
async function getViewColumns(pool, view) {
  ...
  return {
    ...
    filters: result.recordset.map((item) => {
      const sqlType = getSqlTypeText(item);
      return toFilterField(item.name, sqlType, item.isNullable);
    }),
  };
}
...
app.listen(port, () => {
  console.log(`ERP report API listening on http://localhost:${port}`);
});
